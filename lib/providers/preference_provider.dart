...
class PreferenceProvider extends ChangeNotifier {
  ...
  void setRoomType(RoomType? t) { _roomType = t; notifyListeners(); }
  void setMaxPrice(int p) { _maxPrice = p; notifyListeners(); }
  ...
  void complete() { _completed = true; notifyListeners(); }
  ...
  int _score(RoomModel r) {
    int s = 0;
    if (_location != null && r.location.toLowerCase().contains(_location!.toLowerCase())) s += 3;
    if (_roomType != null && r.type == _roomType) s += 3;
    if (_maxPrice != null && r.price <= _maxPrice!) s += 2;
    for (final a in _amenities) {
      if (r.amenities.any((ra) => ra.toLowerCase().contains(a.toLowerCase()))) s += 1;
    }
    return s;
  }
}
