...
class _AdminReportScreenState extends State<AdminReportScreen> {
  ...
  // Firestore reports thật
  List<ReportModel> _firestoreReports = [];
  bool _loadingReports = true;

  @override
  void initState() {
    super.initState();
    _loadFirestoreReports();
  }

  Future<void> _loadFirestoreReports() async {
    try {
        ...
        children: [
          Row(
            children: [
              ...
              const Text(
                'Báo cáo từ người dùng (Firestore)',
                style: TextStyle(
                  fontSize: 15,
              ...
              if (_loadingReports)
                ...
                Text(
                  '${_firestoreReports.length} báo cáo',
                  style: const TextStyle(
                      fontSize: 13, color: Color(0xFF7A8898)),
                ),
              const SizedBox(width: 8),
              IconButton(
                onPressed: _loadFirestoreReports,
          ...
          if (_loadingReports)
          ...
          else if (_firestoreReports.isEmpty)
            const Padding(
              padding: EdgeInsets.symmetric(vertical: 16),
              child: Center(
                child: Text(
                  'Chưa có báo cáo nào',
                  style: TextStyle(color: Color(0xFF7A8898), fontSize: 14),
                ),
              ),
            )
          else
            ListView.separated(
              ...
              itemCount: _firestoreReports.length,
              separatorBuilder: (_, __) =>
                  const Divider(height: 1, color: Color(0xFFE3EBF5)),
              itemBuilder: (context, i) {
                final r = _firestoreReports[i];
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Phòng: ${r.roomId} • ${r.createdAt.day}/${r.createdAt.month}/${r.createdAt.year}',
                              style: const TextStyle(
                                  fontSize: 12, color: Color(0xFF7A8898)),
                            ),
                      ...
                      if (isPending)
                        Row(
                          ...
                          children: [
                            TextButton(
                              onPressed: () =>
                                  _updateReportStatus(r.id, 'resolved'),
                            ...
                            TextButton(
                              onPressed: () =>
                                  _updateReportStatus(r.id, 'dismissed'),
                              style: TextButton.styleFrom(
