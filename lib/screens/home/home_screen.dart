...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<String> _categories = ['Tất cả', 'Chung cư', 'Phòng trọ', 'Nhà riêng', 'Biệt thự'];
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            // Search bar (hoạt động được)
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 6),
              child: Container(
                decoration: BoxDecoration(
                  color: Colors.white, borderRadius: BorderRadius.circular(16),
                  boxShadow: [BoxShadow(color: AppColors.blue.withOpacity(0.06), blurRadius: 10, offset: const Offset(0, 4))],
                ),
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: _searchQuery.isNotEmpty
                        ...
                        : GestureDetector(
                            ...
                            child: const Icon(Icons.tune_rounded, color: AppColors.teal, size: 20)),
            ...
            // Banner Cards (3 card auto-scroll)
            _buildBannerCards(),

            const SizedBox(height: 16),

            // Filter chips (dời xuống đây)
            ...
            SizedBox(
              ...
              child: filtered.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.search_off_rounded, size: 60, color: AppColors.teal.withOpacity(0.2)),
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) => RoomCard(
                        room: filtered[i],
                        isHorizontal: true,
                        onFavoriteTap: () => context.read<RoomProvider>().toggleFavorite(filtered[i].id),
                        onTap: () => Navigator.push(context,
                            MaterialPageRoute(builder: (_) => RoomDetailScreen(room: filtered[i]))),
                      ),
                    ),
            ),

            // Phòng gần đây
            SectionTitle(title: 'Phòng gần đây', actionText: 'Xem bản đồ', onActionTap: () {}),
            ListView.builder(
              shrinkWrap: true,
              ...
              itemBuilder: (_, i) {
                ...
                return RoomCard(
                  room: room,
                  onFavoriteTap: () => context.read<RoomProvider>().toggleFavorite(room.id),
                  onTap: () => Navigator.push(context,
            ...
            return Padding(
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  boxShadow: [BoxShadow(color: b.gradient.first.withOpacity(0.35), blurRadius: 16, offset: const Offset(0, 6))],
                ),
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                        Container(
                          ...
                          child: Text(b.badge, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1)),
                        ),
                        const SizedBox(height: 6),
                        Text(b.title, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900)),
                        const SizedBox(height: 4),
                        Text(b.subtitle, style: TextStyle(color: Colors.white.withOpacity(0.85), fontSize: 12, height: 1.4)),
                      ])),
                    ]),
                  ),
                ]),
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              child: Text(_categories[i],
                  style: TextStyle(color: sel ? Colors.white : AppColors.textSecondary, fontWeight: FontWeight.w700, fontSize: 13)),
            ),
          );
        },
      ),
...
class _BannerData {
  ...
  final String badge;
  const _BannerData({required this.gradient, required this.icon, required this.title, required this.subtitle, required this.badge});
}
