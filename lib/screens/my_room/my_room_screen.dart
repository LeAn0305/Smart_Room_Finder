...
class _MyRoomScreenState extends State<MyRoomScreen> with SingleTickerProviderStateMixin {
  ...
  List<RoomModel> get _activeRooms => _applySort(_rooms.where((r) => r.isActive && !r.isDraft).toList());
  List<RoomModel> get _hiddenRooms => _applySort(_rooms.where((r) => !r.isActive && !r.isDraft).toList());
  List<RoomModel> get _draftRooms => _rooms.where((r) => r.isDraft).toList();

  List<RoomModel> _applySort(List<RoomModel> list) {
    List<RoomModel> filtered = _searchQuery.isEmpty
        ? list
        : list.where((r) => r.title.toLowerCase().contains(_searchQuery) || r.address.toLowerCase().contains(_searchQuery)).toList();
    switch (_sortBy) {
      ...
      case 'views': filtered.sort((a, b) => b.viewCount.compareTo(a.viewCount)); break;
      case 'newest': filtered.sort((a, b) => (b.postedAt ?? DateTime(0)).compareTo(a.postedAt ?? DateTime(0))); break;
    }
    return filtered;
  }
  ...
  void _deleteRoom(RoomModel room) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Xóa phòng?', style: TextStyle(fontWeight: FontWeight.w700)),
  ...
  void _editRoom(RoomModel room) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => PostRoomScreen(editRoom: room)))
        .then((updated) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Text('Sắp xếp theo', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
          const SizedBox(height: 16),
          ...options.map((o) {
            final sel = _sortBy == o.$1;
            return GestureDetector(
              onTap: () { setState(() => _sortBy = o.$1); Navigator.pop(context); },
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: sel ? AppColors.teal : Colors.transparent, width: 1.5),
                ),
                child: Row(children: [
                  Icon(o.$2, color: sel ? AppColors.teal : AppColors.textSecondary, size: 20),
                  const SizedBox(width: 12),
                  Text(o.$3, style: TextStyle(fontWeight: FontWeight.w600, color: sel ? AppColors.tealDark : AppColors.textPrimary)),
                  const Spacer(),
                  if (sel) const Icon(Icons.check_circle_rounded, color: AppColors.teal, size: 20),
                ]),
              ),
            );
          }),
  ...
  Widget _buildHeader() {
    return Padding(
      ...
      child: Row(children: [
        ...
        const Expanded(child: Text('Phòng trọ của tôi',
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: const Icon(Icons.sort_rounded, color: AppColors.teal, size: 20),
          ),
        ),
      ]),
    );
  ...
  Widget _buildStats() {
    final totalViews = _rooms.fold(0, (s, r) => s + r.viewCount);
    final totalContacts = _rooms.fold(0, (s, r) => s + r.contactCount);
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
      child: Row(children: [
        _statChip(Icons.home_work_rounded, '${_rooms.length}', 'Tổng phòng', AppColors.teal),
        ...
        const SizedBox(width: 8),
        _statChip(Icons.warning_amber_rounded, '${_rooms.where((r) => r.isExpired).length}', 'Hết hạn', Colors.orange),
      ]),
    );
  }
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      child: TabBar(
        ...
        indicator: BoxDecoration(color: AppColors.teal, borderRadius: BorderRadius.circular(12)),
        ...
        tabs: [
          Tab(text: 'Đang đăng (${_activeRooms.length})'),
          Tab(text: 'Đã ẩn (${_hiddenRooms.length})'),
          Tab(text: 'Nháp (${_draftRooms.length})'),
        ],
      ),
    );
  }
  ...
  Widget _buildRoomList(List<RoomModel> rooms, {required String tab}) {
    if (rooms.isEmpty) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(tab == 'active' ? Icons.home_work_outlined : tab == 'hidden' ? Icons.visibility_off_outlined : Icons.drafts_outlined,
            size: 64, color: AppColors.textSecondary.withOpacity(0.3)),
        const SizedBox(height: 12),
        Text(tab == 'active' ? 'Chưa có phòng đang đăng' : tab == 'hidden' ? 'Không có phòng bị ẩn' : 'Không có bản nháp',
  ...
  Widget _buildRoomCard(RoomModel room) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: isExpiringSoon ? Border.all(color: Colors.orange, width: 1.5) : null,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Stack(children: [
          ...
          Positioned(top: 10, left: 10, child: Container(
            ...
            decoration: BoxDecoration(
              color: room.isDraft ? Colors.grey : room.isActive ? Colors.green : Colors.orange,
              borderRadius: BorderRadius.circular(8),
            ),
            child: Text(room.isDraft ? 'Nháp' : room.isActive ? 'Đang đăng' : 'Đã ẩn',
          ...
          if (isExpiringSoon)
            Positioned(top: 10, right: 10, child: Container(
              ...
              child: Text('Còn $daysLeft ngày', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700)),
            )),
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              if (room.area != null) ...[
                const SizedBox(width: 12),
                _miniStat(Icons.square_foot_rounded, '${room.area}m²', AppColors.teal),
              ],
            ...
            // Action buttons
            Row(children: [
              _actionBtn(Icons.edit_rounded, 'Sửa', AppColors.teal, () => _editRoom(room)),
              const SizedBox(width: 6),
              _actionBtn(
                room.isActive ? Icons.visibility_off_rounded : Icons.visibility_rounded,
                room.isActive ? 'Ẩn' : 'Hiện',
                room.isActive ? Colors.orange : Colors.green,
                () => _toggleActive(room),
              ),
              const SizedBox(width: 6),
              _actionBtn(Icons.copy_rounded, 'Nhân bản', Colors.purple, () => _duplicateRoom(room)),
              const SizedBox(width: 6),
              if (room.isExpired || room.daysLeft <= 5)
                _actionBtn(Icons.autorenew_rounded, 'Gia hạn', Colors.blue, () => _renewRoom(room))
              else
                _actionBtn(Icons.delete_outline_rounded, 'Xóa', Colors.redAccent, () => _deleteRoom(room)),
            ]),
          ]),
        ),
      ]),
