...
class _SearchResultScreenState extends State<SearchResultScreen> {
  ...
  final List<String> _locations = [
    'Tất cả', 'Quận 1', 'Quận 3', 'Quận 7', 'Quận 10',
  ...
  /// Normalize chuỗi: lowercase + bỏ dấu tiếng Việt để so sánh không phân biệt dấu.
  ...
  void _applyFilters() {
    ...
    setState(() {
      _results = all.where((r) {
        ...
        // Loại phòng
        if (_selectedType != 'Tất cả' && r.type != typeMap[_selectedType]) {
          return false;
        }
  ...
  void _showFilterSheet() {
    // Copy state hiện tại vào biến tạm để sheet chỉnh mà không ảnh hưởng ngay
    String sheetType = _selectedType;
    String sheetLocation = _selectedLocation;
    String sheetPrice = _selectedPrice;
    final Set<String> sheetAmenities = Set.from(_selectedAmenities);
        ...
        builder: (ctx, setSheetState) => Container(
          ...
          child: Column(
            children: [
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        setSheetState(() {
                          sheetType = 'Tất cả';
                          sheetLocation = 'Tất cả';
                          sheetPrice = 'Tất cả';
                          sheetAmenities.clear();
                        });
                      },
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: _locations.map((l) {
                          final sel = sheetLocation == l;
                          return _filterChip(
                            label: l,
                            selected: sel,
                            onTap: () => setSheetState(() => sheetLocation = l),
                          );
                        }).toList(),
                      ),
                      const SizedBox(height: 20),
                      ...
                      Wrap(
                        ...
                        children: _priceRanges.map((p) {
                          final sel = sheetPrice == p.$1;
                          return _filterChip(
                            label: p.$1,
                            selected: sel,
                            onTap: () => setSheetState(() => sheetPrice = p.$1),
                          );
                        }).toList(),
                      ),
                      const SizedBox(height: 20),
                      ...
                      Wrap(
                        ...
                        children: _amenities.map((a) {
                          final sel = sheetAmenities.contains(a.$1);
                          return GestureDetector(
                            onTap: () => setSheetState(() {
                              if (sel) {
                                sheetAmenities.remove(a.$1);
                              } else {
                                sheetAmenities.add(a.$1);
                              }
                            }),
              ...
              // Nút áp dụng — sync state từ sheet về widget rồi mới pop
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      // Sync filter từ sheet về widget state
                      setState(() {
                        _selectedType = sheetType;
                        _selectedLocation = sheetLocation;
                        _selectedPrice = sheetPrice;
                        _selectedAmenities
                          ..clear()
                          ..addAll(sheetAmenities);
                      });
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
              color: selected ? AppColors.teal : Colors.grey[200]!, width: 1.5),
        ),
        child: Text(label,
            style: TextStyle(
              fontSize: 13,
                ...
                child: ListView(
                  ...
                  children: [
                    if (_selectedType != 'Tất cả')
                      _activeChip(_selectedType, () => setState(() {
                            _selectedType = 'Tất cả';
                            _applyFilters();
                          })),
                    if (_selectedLocation != 'Tất cả')
                      _activeChip(_selectedLocation, () => setState(() {
                            _selectedLocation = 'Tất cả';
                            _applyFilters();
                          })),
                    if (_selectedPrice != 'Tất cả')
                      _activeChip(_selectedPrice, () => setState(() {
                            _selectedPrice = 'Tất cả';
                            _applyFilters();
                          })),
                    ..._selectedAmenities.map((a) => _activeChip(a, () => setState(() {
